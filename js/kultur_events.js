(function ($) {
  ...
  Drupal.behaviors.kultur_events_filtering = {
    attach: function (context, settings) {
      // Load filters.
      $('#kultur-events-filters', context).each(function () {
        $.getJSON(Drupal.settings.basePath + 'arrangementer-liste/filters/load', function (content) {
          $('#kultur-events-filters', context).html(content);
        });
      });
      ...
      $('#edit-og-group-ref-target-id-entityreference-filter-wrapper', context).once(function() {
        let items = $('#edit-og-group-ref-target-id-entityreference-filter-wrapper', context).find(':checkbox');
        if (items.length > 0) {
          $.each(items, function (i, val) {
            $(val).on('click', function () {
              let status = $(val).attr('checked');
              ...
              let categories = $('#edit-field-ding-event-category-tid-wrapper', context).find(':checkbox');
              ...
              let path = 'filter_categories' + params;
        ...
        if (categories.length > 0) {
          $.each(categories, function(i, val) {
            $(val).click(function() {
              ...
              let libraries = $('#edit-og-group-ref-target-id-entityreference-filter-wrapper', context).find(':checkbox');
              ...
              let path = 'filter_libraries' + params;
